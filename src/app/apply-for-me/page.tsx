...
import { 
  ...
  Search, ChevronRight, Lock, Zap, FileText, IndianRupee, X
} from "lucide-react";
...
function FormCard({ form, isHighlighted = false }: { form: any; isHighlighted?: boolean }) {
  ...
  const serviceCharge = form.fees_structure?.[0]?.fees?.serviceCharge ?? "50";
          ...
          <>
            {submitted && (
              <div className="bg-emerald-50 dark:bg-emerald-950/20 border-2 border-emerald-200 dark:border-emerald-800/50 rounded-3xl p-6 text-center space-y-4 animate-in zoom-in-95">
                ...
                <div className="inline-block bg-white dark:bg-zinc-900 border border-gray-100 dark:border-zinc-800 px-6 py-2 rounded-2xl">
            ...
            <div className="bg-white dark:bg-zinc-950 rounded-2xl border border-gray-200 dark:border-zinc-800 shadow-sm overflow-hidden">
              ...
              {searchQuery.length > 0 && searchQuery.length < 2 && (
                <div className="px-5 pb-4 text-xs font-bold text-gray-400">Type at least 2 characters to search...</div>
              )}
            </div>
              ...
              <div className="space-y-3">
                ...
                {searchResults.length > 0 ? (
                  ...
                  <div className="bg-white dark:bg-zinc-950 rounded-2xl border border-dashed border-gray-200 dark:border-zinc-800 p-8 text-center space-y-4">
                    ...
                    <a
                      href={`https://wa.me/918118029532?text=Hello+Rojgar+Suvidha+Team!+Kya+${encodeURIComponent(debouncedQuery)}+form+available+hai?+Please+mujhe+bataye.`}
                      target="_blank"
                      rel="noreferrer"
            ...
            {!hasSearchQuery && (
              <div className="space-y-4">
                ...
                {formsLoading ? (
                  ...
                  <div className="bg-white dark:bg-zinc-950 rounded-2xl border border-dashed border-gray-200 dark:border-zinc-800 p-8 text-center">
                    <p className="text-sm font-bold text-gray-400">Abhi koi active form nahi hai.</p>
                    <p className="text-xs text-gray-400 mt-1">Admin jald hi naye forms add karega. Search karke check kar sakte hain.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              orders.map((order) => (
                <div key={order.id} className="bg-white dark:bg-zinc-950 rounded-3xl border border-gray-100 dark:border-zinc-900 p-5 shadow-sm space-y-4 animate-in fade-in">
                  ...
                  <div className="flex justify-end gap-2 border-t border-gray-50 dark:border-zinc-900/50 pt-4 text-xs font-bold">
                    <a href={`https://wa.me/918118029532?text=Hello+Rojgar+Suvidha,+my+order+ID+is+${order.payment_id}.+Please+update+the+status.`} target="_blank" rel="noreferrer"
