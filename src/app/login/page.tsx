...
function LoginContent() {
  ...
  // Auto-submit OTP (or just handle change if length varies)
  const handleOtpChange = (val: string) => {
    // allow up to 8 digits
    const cleanVal = val.replace(/\D/g, '').slice(0, 8);
    setOtp(cleanVal);
    // Remove auto-submit to prevent early submission if it's 8 digits but starts with 6
    // The user can just click "Verify & Login" manually.
  };
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        ...
        <p className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400 font-medium">
          Sign in or create an account to access your digital locker and saved jobs.
        </p>
      </div>
